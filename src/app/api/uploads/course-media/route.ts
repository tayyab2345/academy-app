...
import { buildStoredCourseMediaFilename } from "@/lib/course-media"
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      target !== "course_syllabus_pdf" &&
      target !== "course_syllabus_image"
    ) {
    ...
    const isPdfTarget = target === "course_syllabus_pdf"
    ...
    const storageResult = await storeDocument(
      buffer,
      buildStoredCourseMediaFilename({
    ...
    console.error("Failed to upload course media:", error)
    return NextResponse.json(
